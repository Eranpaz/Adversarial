...
def generator(z_x_ph):
    with tf.name_scope('gen_fc'):
        weights=tf.Variable(tf.truncated_normal([z_x_ph.shape[1].value,cfg.MNIST.VAE.GEN.FC.size],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
    ...
    with tf.name_scope('gen_deconv2'):
        weights=tf.Variable(tf.truncated_normal([cfg.MNIST.VAE.GEN.DECONV2.kernel_size,cfg.MNIST.VAE.GEN.DECONV2.kernel_size,1,cfg.MNIST.VAE.GEN.DECONV2.num_channels],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
    ...
    with tf.name_scope('gen_deconv3'):
        weights=tf.Variable(tf.truncated_normal([cfg.MNIST.VAE.GEN.DECONV3.kernel_size,cfg.MNIST.VAE.GEN.DECONV3.kernel_size,1,cfg.MNIST.VAE.GEN.DECONV3.num_channels],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
    ...
    with tf.name_scope('gen_x'):
        weights=tf.Variable(tf.truncated_normal([cfg.MNIST.VAE.GEN.X.kernel_size,cfg.MNIST.VAE.GEN.X.kernel_size,1,cfg.MNIST.VAE.GEN.X.num_channels],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
...
def discriminator(x_tilde_ph):
    ...
    with tf.name_scope('dis_conv2'):
        weights=tf.Variable(tf.truncated_normal([cfg.MNIST.VAE.DIS.CONV2.kernel_size,cfg.MNIST.VAE.DIS.CONV2.kernel_size,1,cfg.MNIST.VAE.DIS.CONV2.num_channels],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
    ...
    with tf.name_scope('dis_conv3'):
        weights=tf.Variable(tf.truncated_normal([cfg.MNIST.VAE.DIS.CONV3.kernel_size,cfg.MNIST.VAE.DIS.CONV3.kernel_size,1,cfg.MNIST.VAE.DIS.CONV3.num_channels],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
    ...
    with tf.name_scope('dis_conv4'):
        weights=tf.Variable(tf.truncated_normal([cfg.MNIST.VAE.DIS.CONV4.kernel_size,cfg.MNIST.VAE.DIS.CONV4.kernel_size,1,cfg.MNIST.VAE.DIS.CONV4.num_channels],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
    ...
    with tf.name_scope('dis_fc'):
        weights=tf.Variable(tf.truncated_normal([flatten.shape[1].value,cfg.MNIST.VAE.DIS.FC.size],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
    ...
    with tf.name_scope('dis_logits'):
        weights=tf.Variable(tf.truncated_normal([cfg.MNIST.VAE.DIS.FC.size,1],stddev=1.0 / math.sqrt(float(IMG_SIZE*IMG_SIZE))),name='weights')
...
def training(SSE_loss, KL_loss, D_loss, G_loss, LL_loss, learning_rate):
    ...
    opt_e = optimizer.minimize(L_e, var_list=e_vars)
    opt_g = optimizer.minimize(L_g, var_list=g_vars)
    opt_d = optimizer.minimize(L_d, var_list=d_vars)
...
def train():
    ...
    opt_e,opt_g, opt_d=training(SSE_loss, KL_loss, D_loss, G_loss, LL_loss,LR)
